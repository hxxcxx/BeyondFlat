...
void GLRenderer::drawMesh(const Mesh& mesh) {
    ...
    glUniform3f(uniforms_.lightDir, 0.5f, 0.8f, 1.0f);
    glUniform3f(uniforms_.ambientColor, 0.3f, 0.3f, 0.35f);
    glUniform3f(uniforms_.diffuseColor, 0.7f, 0.7f, 0.65f);
...
void GLRenderer::drawAxes(float length) {
    drawLine(Vector3d::Zero(), Vector3d(length, 0, 0), Color4f(1, 0.2f, 0.2f, 1).toABGR());
    drawLine(Vector3d::Zero(), Vector3d(0, length, 0), Color4f(0.2f, 1, 0.2f, 1).toABGR());
    drawLine(Vector3d::Zero(), Vector3d(0, 0, length), Color4f(0.3f, 0.3f, 1, 1).toABGR());
}

void GLRenderer::drawGrid(float size, int divisions) {
    float step = size / divisions;
    uint32_t color = Color4f(0.3f, 0.3f, 0.35f, 0.5f).toABGR();
    uint32_t axisColor = Color4f(0.5f, 0.5f, 0.55f, 0.7f).toABGR();
